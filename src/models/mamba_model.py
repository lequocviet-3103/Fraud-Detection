...
# ── Mamba import (no patching — rely on installed mamba-ssm + causal-conv1d) ─
def _import_mamba():
    """Lazy import of mamba-ssm.

    On Kaggle the prebuilt wheels (mamba-ssm + causal-conv1d) load correctly
    against the system CUDA runtime, so no slow-path fallback is needed.
    If the CUDA extensions fail to load, we still attempt import and let
    Mamba.forward raise its own diagnostic.
    """
    try:
